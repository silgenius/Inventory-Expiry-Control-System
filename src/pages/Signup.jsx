...
import { Eye, EyeOff, Boxes, Check, PackageSearch, CalendarClock, BellRing } from 'lucide-react'
...
function PasswordRequirement({ met, label }) {
  return (
    <li className={`flex items-center gap-1.5 text-xs ${met ? 'text-emerald-600' : 'text-navy-400'}`}>
...
export default function Signup() {
  ...
  function validate() {
    ...
    if (!values.businessAddress.trim()) nextErrors.businessAddress = 'Business address is required.'
    ...
    if (!passwordChecks.length || !passwordChecks.number || !passwordChecks.uppercase) {
  ...
  return (
    <div className="min-h-screen bg-navy-50/50 dark:bg-navy-900 lg:flex">
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-navy-900 p-12 text-white lg:flex">
        ...
        <div>
          <h2 className="text-3xl font-bold leading-tight">Inventory Expiry Control System</h2>
          ...
          <div className="mt-10 space-y-4">
            <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3.5 backdrop-blur">
              ...
              <span className="text-sm text-navy-100">Track every product batch in one place</span>
            </div>
            <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3.5 backdrop-blur">
              <CalendarClock className="h-5 w-5 text-emerald-400" />
              <span className="text-sm text-navy-100">Monitor days remaining before expiry</span>
            </div>
            <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3.5 backdrop-blur">
              <BellRing className="h-5 w-5 text-emerald-400" />
              <span className="text-sm text-navy-100">Get alerted before it's too late</span>
            </div>
          </div>
        </div>

        <p className="text-xs text-navy-400">© {new Date().getFullYear()} IECS. All rights reserved.</p>
      </div>
          ...
          <div className="relative z-10 -mt-8 px-4 pb-10 lg:mt-0 lg:px-0 lg:pb-0">
            <div className="rounded-3xl border border-navy-100 bg-white p-5 shadow-popover dark:border-navy-700 dark:bg-navy-800 sm:p-6 lg:rounded-2xl lg:border lg:shadow-card">
              <div className="hidden lg:block">
                <h1 className="text-xl font-bold text-navy-900 dark:text-navy-50">Create Your Business Account</h1>
              ...
              <form onSubmit={handleSubmit} noValidate className="mt-2 space-y-5 lg:mt-6">
                <FormSection step={1} title="Business details">
                  ...
                  <Input
                    ...
                    onChange={(event) => update('administratorName', event.target.value)}
                  />
                </FormSection>
                ...
                <FormSection step={2} title="Sign-in details">
                  ...
                  <div className="relative">
                    ...
                    <ul className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
                      <PasswordRequirement met={passwordChecks.length} label="Minimum 8 characters" />
                      <PasswordRequirement met={passwordChecks.number} label="At least one number" />
                      <PasswordRequirement met={passwordChecks.uppercase} label="At least one uppercase letter" />
                    </ul>
                  </div>
                ...
                <div>
                  ...
                  {errors.agreeToTerms && <p className="mt-1 text-xs font-medium text-red-600">{errors.agreeToTerms}</p>}
                </div>
      ...
      >
        Welcome to IECS. Your business account has been created. Sign in to explore your dashboard.
      </Modal>
    </div>
  )
}
